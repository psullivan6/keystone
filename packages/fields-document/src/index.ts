...
function normaliseRelationships(
  ...
  if (configRelationships) {
    Object.keys(configRelationships).forEach(key => {
      ...
      if (meta.models[relationship.modelKey] === undefined) {
        throw new Error(
          `An inline relationship ${relationship.label} (${key}) in the field at ${meta.modelKey}.${meta.fieldKey} has listKey set to "${relationship.modelKey}" but no list named "${relationship.modelKey}" exists.`
        );
      }
