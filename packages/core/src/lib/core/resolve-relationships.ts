...
export function resolveRelationships(
  ...
  for (const [modelKey, fields] of Object.entries(models)) {
    ...
    for (const [fieldPath, { dbField: field }] of Object.entries(fields.fields)) {
      ...
      const foreignUnresolvedModel = models[field.model];
      ...
      if (field.field) {
        ...
        const foreignRef = `${field.model}.${field.field}`;
        ...
        if (foreignField.model !== modelKey) {
          throw new Error(
            `The relationship field at ${localRef} points to ${foreignRef} but ${foreignRef} points to the model ${foreignField.model} rather than ${modelKey}`
          );
        }
        ...
        let [leftRel, rightRel] = sortRelationships(
          ...
          { modelKey: field.model, fieldPath: field.field, field: foreignField }
        );
      ...
      if (foreignUnresolvedModel.fields[foreignFieldPath]) {
        throw new Error(
          `The relationship field at ${modelKey}.${fieldPath} points to the model ${field.model}, Keystone needs to a create a relationship field at ${field.model}.${foreignFieldPath} to support the relationship at ${modelKey}.${fieldPath} but ${field.model} already has a field named ${foreignFieldPath}`
        );
      }

      if (field.mode === 'many') {
        const relationName = field.relationName ?? `${modelKey}_${fieldPath}`;
        resolvedModels[field.model][foreignFieldPath] = {
          kind: 'relation',
          mode: 'many',
          model: modelKey,
          field: fieldPath,
