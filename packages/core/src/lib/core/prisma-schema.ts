...
function printField(
  ...
  if (field.kind === 'relation') {
    if (field.mode === 'many') {
      return `${fieldPath} ${field.model}[] @relation("${field.relationName}")`;
    }
    if (field.foreignIdField.kind === 'none') {
      return `${fieldPath} ${field.model}? @relation("${field.relationName}")`;
    }
    const relationIdFieldPath = `${fieldPath}Id`;
    const relationField = `${fieldPath} ${field.model}? @relation("${field.relationName}", fields: [${relationIdFieldPath}], references: [id])`;
    const foreignIdField = models[field.model].resolvedDbFields.id;
    assertDbFieldIsValidForIdField(field.model, foreignIdField);
